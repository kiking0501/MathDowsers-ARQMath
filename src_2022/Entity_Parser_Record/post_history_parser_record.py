...
class PostHistoryParser:
    ...
    def __init__(self, xml_post_history_file_path):
        self.map_of_edits = {}
        for ind, attr_dic in enumerate(xmliter(xml_post_history_file_path, 'row')):
            if RECORD_NUM['post_history'] is not None and ind >= RECORD_NUM['post_history']:
                break
    ...
    @classmethod
    @CACHE(PARSER_PKL['post_history'])
    def load(cls, xml_post_history_file_path):
        print("Initiating...")
        parser = cls(xml_post_history_file_path)
        return parser
