...
import os
from config import DATA_PATH, ARQM_PREPRO_PATH, WIKI_DATA_PATH
...
def create_MSE_tags(output_file="MSE_tags.txt"):
    ...
    with open(os.path.join(ARQM_PREPRO_PATH, output_file), "w") as f:
...
def create_tag_keywords(input_file="MSE_tags.txt", output_file="MSE_tag_keywords.txt"):
    ...
    """
    count_pairs = []
    with open(os.path.join(ARQM_PREPRO_PATH, input_file)) as f:
        for l in f.readlines():
