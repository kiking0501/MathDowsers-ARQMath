...
TASK1_TOPICS = {
    2020: os.path.join(
        ARQM_EXPERIMENTS_PATH, "topics", "ARQMath_2021", "task1-topics-2020.json"),
    2021: os.path.join(
        ARQM_EXPERIMENTS_PATH, "topics", "ARQMath_2021", "task1-topics-2021.json")
}

TASK2_TOPICS = {
    2020: os.path.join(
        ARQM_EXPERIMENTS_PATH, "topics", "ARQMath_2021", "task2-formula-2020.json"),
    2021: os.path.join(
        ARQM_EXPERIMENTS_PATH, "topics", "ARQMath_2021", "task2-formula-2021.json")
}

FORMULA_AS_TOPICS = {
    2020: os.path.join(ARQM_EXPERIMENTS_PATH, "topics", "ARQMath_2021", "formula-as-topics-slt-2020.json"),
    2021: os.path.join(ARQM_EXPERIMENTS_PATH, "topics", "ARQMath_2021", "formula-as-topics-slt-2021.json")
}
...
def create_converted_slt_representation(year):
    ...
    from prepro_create_custom_formula_representation import naive_clean_pmml
    slt_tsv = read_task_formula_tsv(
        year, "slt_original", groupby_list=["id"])
    ...
    for formula_id in tqdm(sort_formula_ids(slt_tsv.keys())):
        ...
        update_slt = naive_clean_pmml(item['formula'].replace("  ", "").replace("> <", "><"))
        if update_slt:
            num_updated += 1
...
def create_task1_topics_slt(year):
    ...
    task1_topics = load_json(
        TASK1_TOPICS[year], verbose=True
    )
    formulaTSVs = {
    ...
    dump_json(task1_topics,
              TASK1_TOPICS[year].partition('.')[0] + "-slt" + ".json")


def create_2020_task1_human_translated():
    """ Get the human-translated parts as query files """
    json_path = os.path.join(ARQM_EXPERIMENTS_PATH, "topics", "ARQMath_2020", "task1-topics-slt.json")
    topic_json = load_json(json_path)

    human_translated = {}
    for topic_id, content in topic_json.items():
        human_translated[topic_id] = content["Human_Translated"]
...
def create_task2_topics_slt(year):
    ...
    task2_topics = load_json(
        TASK2_TOPICS[year], verbose=True
    )
    formulaTSVs = {
        formula_type: read_task_formula_tsv(
            year, formula_type, groupby_list=["topic_id", "id"])
    ...
    dump_json(task2_topics,
              TASK2_TOPICS[year].partition('.')[0] + "-slt" + ".json")


def create_formula_as_topics(year):
    """ Create topic-like files for for each appearing formula in Task 1 topics """
    ...
    formula_as_topics = {}
    task_formula_tsv = read_task_formula_tsv(
        year, "slt", groupby_list=["id"]
    )
    for formula_id, items in task_formula_tsv.items():
        formula_as_topics[formula_id] = {"Question": "", "Title": ""}
        formula_as_topics[formula_id][items[0]["type"].capitalize()] = _clean_slt_formula(
            items[0]["formula"], items[0])

    dump_json(formula_as_topics, FORMULA_AS_TOPICS[year])


def separate_json(json):
    """ Save each topic as one single JSON file """
    output_folder = json.rpartition(".")[0]
    os.makedirs(output_folder, exist_ok=True)

    json_dict = load_json(json)
    name = json.rpartition("/")[2].partition(".")[0]
    for k, v in json_dict.items():
        dump_json(
            v,
            os.path.join(output_folder, name + "-" + k + ".json")
        )
...
if __name__ == '__main__':
    ...
    for xml_path, output_path in (
        (os.path.join(ARQM_TASK1_PATH, "ARQMath_2020", "Topics", "Topics_V2.0.xml"),
         TASK1_TOPICS[2020]),

        (os.path.join(ARQM_TASK1_PATH, "ARQMath_2021", "Topics", "Topics_Task1_2021_V1.1.xml"),
         TASK1_TOPICS[2021]),
    ):
    ...
    # create_2020_task1_human_translated()

    #### Task2
    for xml_path, output_path in (
        (os.path.join(ARQM_TASK2_PATH, "ARQMath_2020", "Topics", "Topics_V1.1.xml"),
         TASK2_TOPICS[2020]),

        (os.path.join(ARQM_TASK2_PATH, "ARQMath_2021", "Topics", "Topics_Task2_2021_V1.1.xml"),
         TASK2_TOPICS[2021]),
    ):
        topics_xml2json(xml_path, output_path)
    for year in (2020, 2021):
    ...
    ### For Holistic Formula Search in Task 1
    # create_formula_as_topics(2020)
    # create_formula_as_topics(2021)

    #### Save topics separately
    # separate_json(
    #     TASK1_TOPICS[2020]
    # )
    # separate_json(
    ...
    # )
    # separate_json(
    #     TASK1_TOPICS[2020].rpartition(".")[0] + "-slt.json"
    # )
    # separate_json(
    #     TASK1_TOPICS[2021].rpartition(".")[0] + "-slt.json"
    # )
