...
function searchQuestion() {
    ...
    window.open("view_result.html?ARQMath_year=" + arqmath_year + "&topic_id=" + topic_id);
}
...
function loadQuestionPanel(formulaDisplayStyle) {
    $("#question-panel").load("gh-pages/question_panel.html", function(){
        $(".q-text").hide();
        $("#formula-display-style").text(formulaDisplayStyle);
        console.log(getFormulaDisplayStyle());
    });
}
...
function writeResult(arqmath_year, topic_id) {
    ...
    getTerms(function (data) {
        ...
        for (let i = 0; i < terms.length; i++) {
            // html += "<ul>";
            html += "<div>";
            if (terms[i][0].startsWith("<math")) {
                html += cleanFormula(terms[i][0]);
            } else {
                html += terms[i][0];
            }
