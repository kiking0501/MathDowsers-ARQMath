# slightly modified version of de_duplicate.py from ARQMath-3
...
def deduplicated_file(file, topic_visual_lst, dic_formula_visual_ids):
    """
    ...
    """
    deduplicated_result, run_id = deduplicate(dic_formula_visual_ids, file, topic_visual_lst)
    prim_dic = {}
    ...
    if (x<0):
        x = 0
    outfile = file[:x+1]+"trec-"+file[x+1:]

    with open(outfile, "w", newline='', encoding="utf-8") as result_file:
...
def main():
    ...
    "/ARQMath 2021 Submission/Z_Task2_2021/"
    """
    ...
    parser.add_argument('files', nargs='+', help='Pattern path matching the submitted files')
    args = parser.parse_args()

    tsv_dir = args.tsv + "/"
    qrel_file_path = args.qre
    dic_formula_visual_ids = read_visual_files(tsv_dir, 6)
    topic_visual_lst = read_qrel(qrel_file_path)

    for file in args.files:
        deduplicated_file(file, topic_visual_lst, dic_formula_visual_ids)
